...
namespace Unity.Services.Vivox
{
    ...
    internal class TextToSpeech : ITextToSpeech
    {
        ...
        public void CancelDestination(TextToSpeechMessageType messageType)
        {
            ReadOnlyCollection<TTSMessage> destCol = GetMessagesFromDestination(messageType);
            if (destCol.Count == 0)
                return;

            _client.TTSInitialize();

            vx_tts_status status = VivoxCoreInstance.vx_tts_cancel_all_in_dest(TTSManagerId, (vx_tts_destination)messageType);
        ...
        public ReadOnlyCollection<TTSMessage> GetMessagesFromDestination(TextToSpeechMessageType messageType)
        {
            return _ttsMessages.Where(m => m.Destination == messageType).ToList().AsReadOnly();
        }
